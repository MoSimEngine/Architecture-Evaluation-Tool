package org.mosim.refactorlizar.architecture.evaluation.size;
...
public class HyperGraphSizeCalculator<T> {
    ...
    public HyperGraphSizeCalculator() {
        mode = CalculationMode.NO_OFFSET;
    }
    ...
    private int getSystemSize(Set<Node<T>> nodes) {
        return mode == CalculationMode.ONE_OFFSET ? nodes.size() + 1 : nodes.size();
    }
